...
Datum:     March 10, 2026
Taggar:    Music Packs
Status:    Draft | Published | Ready for update
...
When a post has Status `Ready for update` in the Notion Blog Posts database:

1. Fetch the post content from Notion (read the updated Brödtext and other fields)
2. In `blog.html`: use `str_replace` to replace the matching `<article id="SLUG">` block with the updated content
3. In `index.html`: use `str_replace` to replace the matching `.blog-card` (update ingress `data-full`, title, tags, date as needed)
4. Commit both files
5. Reset the post's Status in Notion back to `Published`

This workflow only applies to already-published posts — not new posts.
...
Notes:
- Always prepend **before** the `<!-- Add new posts above this line -->` comment — this is the anchor point
- Omit the `<img>` tag entirely if no Bild-URL
- `id="SLUG"` must exactly match the anchor in index.html (`blog.html#SLUG`)
...
> **Reminder:** If Martin mentions an image for a post, check that it exists in the repo before referencing it. If it doesn't, remind Martin to upload it first.
