...
- **Tracks:** Grey Morning Mist, The Hollow Trunk, Faded Petals, Shadows Beneath Ferns, A Place to Rest, Corrupted Sap, Forgotten Shrine, The Last Leaf Falls, Drowning in Moss, Roots of Memory
- **Current store links:** store profile links (follow buttons) — replace with actual listing URLs at release
...
4. Add SoundCloud playlist URL and update `FALLBACK_TITLES` / `FALLBACK_DURATIONS` for `cp4`
5. Add SoundCloud player: new iframe `sc-4`, player div `cp4`
6. Add `cp4` config to `CONFIGS` array in JS
7. Update Guide 07 in Notion (ID: `32bbf76e-302d-81fa-8493-f1ed576ce381`) — move the 003 task from "Aktiva uppgifter" to "Klara uppgifter"

---

## In Pipeline (not yet on site)

### Crystal & Frost: Intimate Winter Exploration
- **Status:** Concept / planning
- **Subtitle:** Intimate winter exploration
- **Style:** Piano (felt), celesta, marimba, small string section — 70% minor
- **Target:** Metroidvania, puzzle games, atmospheric adventure
- **Planned tracks:** Frozen Breath, The Ice Library, Shards of Memory, Beneath the Permafrost, Snowdrift Waltz, The Rising Blizzard, Glacial Echoes, Brittle Victory, Whiteout Silence, Thaw
...
| Player ID | Pack |
|---|---|
| `cp1` / `sc-1` | 001 Atlas Companion |
| `cp2` / `sc-2` | 002 Orchestral Exploration Pack |
| `cp3` / `sc-3` | Tantleken (film) |
| `cp4` / `sc-4` | 003 Echoes of the Silent Woods (reserved for release) |
